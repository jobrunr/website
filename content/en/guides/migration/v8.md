...
- {{< label version="enterprise" >}}JobRunr Pro Enterprise{{< /label >}} [Multi-Cluster Dashboard](#multi-cluster-dashboard)
...
- {{< label version="professional" >}}JobRunr Pro{{< /label >}} Exposed transaction package name change: from `org.jobrunr.exposed.transaction.sql` to `org.jobrunr.kotlin.storage.sql.exposed`
...
Dynamic queues can now be paused and resumed from the Dashboard or programmatically. 
