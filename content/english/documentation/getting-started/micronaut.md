...
> [!IMPORTANT]
> `database.type: mem` does not survive restarts. Before going to production, remove it and configure a real database. JobRunr will pick up the existing Micronaut `DataSource` automatically. See [Storage]({{< ref "documentation/storage" >}}).
