---
title: "JobRunr Cloud | Run background jobs on spot, save 60 to 80%"
description: "JobRunr Cloud runs your background jobs on the cheapest spot instances across AWS and Google Cloud. Save 60 to 80% on your cloud bill. CPU and GPU both supported, starting with CPU. Launching September 2026. Join the private beta waitlist."
layout: "cloud"
sitemap:
  priority: 0.1
  changeFreq: never
...
hero:
  ...
  title_start: "Background jobs on spot."
  title_highlight: "60 to 80% off your cloud bill."
  description: "JobRunr Cloud finds the cheapest spot instance in your own AWS and Google Cloud accounts, dispatches your job there, and re-runs it if the instance gets preempted."
...
how_it_works:
  ...
  description: "Tell JobRunr Cloud when you need the result and what you're willing to pay. We watch spot pricing across AWS and Google Cloud, run your job when it fits, and re-dispatch if the instance gets preempted. The API mirrors the carbon-aware jobs API you may already know from JobRunr Pro."
...
email_section:
  ...
  package_items:
    ...
    - "Free use of the JobRunr Cloud platform for the first 90 days"
    - "1:1 onboarding call with the founding team"

audience_section:
  title: "If any of this sounds like you, the waitlist is the right place"
  description: "JobRunr Cloud is for the teams already feeling the cost."
  items:
...
accordion:
  ...
  list:
    - title: "When does JobRunr Cloud launch?"
    ...
    - title: "Where does my compute actually run? Does my data leave my cloud?"
      description: "Your jobs run in your own AWS and Google Cloud accounts. You connect them once with a service role, and JobRunr Cloud dispatches work into your environment. The data stays in your VPC, your security and compliance posture doesn't change, and you keep your existing cloud relationships and discounts. We're the broker, not the host."
    ...
    - title: "Will it work with the JobRunr OSS version?"
      description: "Yes. JobRunr Cloud connects to your existing JobRunr or JobRunr Pro instance over a single outbound WebSocket. You don't change how you define jobs. The cloud features are opt-in per job."
    - title: "How will pricing work?"
      description: "You pay AWS and Google Cloud directly for the compute, just like today. You keep the full 60 to 80 percent spot saving. JobRunr Cloud charges a flat platform fee for the orchestration: webhook relay, multi-provider broker, failover, cost optimization, dashboard. Free tier covers small workloads. Pro and Enterprise tiers add higher limits, dedicated infrastructure, and SLAs. Final numbers come out of the design partner conversations."
    - title: "Is this replacing JobRunr Pro?"
      description: "No. JobRunr Pro keeps doing what it does inside your app: priority queues, batches, workflows, multi-cluster dashboard. JobRunr Cloud handles what needs to happen outside your app: the public webhook, the multi-provider broker, the cost optimization."
